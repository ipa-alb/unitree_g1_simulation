...
class UnitreeSdk2Bridge:
    ...
    def PublishLowState(self, obs: Dict[str, any]):
        ...
        if self.joystick is not None or self.use_keyboard:
            import struct
    ...
    def PublishWirelessController(self):
        if self.joystick is not None:
            ...
            self._publish_keyboard_controller()
    ...
    def SetupJoystick(self, device_id=0, js_type="xbox"):
        ...
        if joystick_count > 0:
            ...
            print("No gamepad detected. Using keyboard control (WASD + arrows in viewer window).")
            self.SetupKeyboard()
            return
    ...
    def SetupKeyboard(self):
        ...
        self.keyboard_state = {}
        self._kb_hold_timeout = 0.15  # seconds before a key is considered released
        # GLFW key code -> gamepad stick mapping
        self.kb_stick_map = {
            glfw.KEY_W: ("ly", 1.0),
            glfw.KEY_S: ("ly", -1.0),
            glfw.KEY_A: ("lx", -1.0),
            glfw.KEY_D: ("lx", 1.0),
            glfw.KEY_UP: ("ry", 1.0),
            glfw.KEY_DOWN: ("ry", -1.0),
            glfw.KEY_LEFT: ("rx", -1.0),
            glfw.KEY_RIGHT: ("rx", 1.0),
        }
        # GLFW key code -> gamepad button mapping
        self.kb_button_map = {
    ...
    def KeyboardCallback(self, key):
        if not self.use_keyboard:
            return
        self.keyboard_state[key] = _time.time()

    def _kb_active(self, key_code):
        t = self.keyboard_state.get(key_code, 0)
        return (_time.time() - t) < self._kb_hold_timeout

    def _publish_keyboard_controller(self):
        lx, ly, rx, ry = 0.0, 0.0, 0.0, 0.0
        for key_code, (axis, value) in self.kb_stick_map.items():
            if self._kb_active(key_code):
                if axis == "lx":
                    lx += value
                elif axis == "ly":
                    ly += value
        ...
        key_state = [0] * 16
        for key_code, btn_name in self.kb_button_map.items():
            if self._kb_active(key_code):
                key_state[self.key_map[btn_name]] = 1
