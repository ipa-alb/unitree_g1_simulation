...
class DefaultEnv:
    ...
    def init_scene(self):
        ...
        # Combined key callback that forwards to elastic band and keyboard bridge
        def _key_callback(key):
            if self.config["ENABLE_ELASTIC_BAND"]:
                self.elastic_band.MujuocoKeyCallback(key)
            if self.unitree_bridge is not None and self.unitree_bridge.use_keyboard:
                self.unitree_bridge.KeyboardCallback(key)
    ...
    def sim_step(self):
        ...
        if self.unitree_bridge.joystick or self.unitree_bridge.use_keyboard:
